...
void mat_mul(double* a, double** b, double* result, int n, int p) {
    ...
    // result = a * b
    int j, k;
    #pragma omp parallel for collapse(2) private(k)  // Parallelize outer loop over 'j'
    for (j = 0; j < p; j++) {
        result[j] = 0.0;
        for (k = 0; k < n; k++)
            result[j] += (a[k] * b[k][j]);
...
void softmax(int n, double* input, double* output) {
    ...
    int i;
    double sum = 0.0;
    #pragma omp parallel for reduction(+:sum)  // Parallelize sum computation
    for (i = 0; i < n; i++)
        sum += exp(input[i]);

    #pragma omp parallel for  // Parallelize softmax computation
    for (i = 0; i < n; i++) 
